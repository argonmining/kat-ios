                ...
                if let kasplexInfo = viewModel.kasplexInfo, !viewModel.isLoading {
                    WidgetDS {
                        VStack(alignment: .leading, spacing: .zero) {
                            ...
                            Text(Formatter.formatToNumber(value: kasplexInfo.tokenTotal))
                                .typography(.numeric3)
                                .lineLimit(1)
                            ...
                            Text(Formatter.formatToNumber(value: kasplexInfo.opTotal))
                                .typography(.numeric3)
                                .lineLimit(1)
                                .padding(.top, Spacing.padding_1)
                            ...
                            Text(Localization.kasplexInfoFeesPaid)
                                .typography(.headline3, color: .textSecondary)
                                .padding(.top, Spacing.padding_2)
                            Text(Formatter.formatToNumber(value: kasplexInfo.feeTotal) + " KAS")
                                .typography(.numeric3)
                                .lineLimit(1)
                                .padding(.top, Spacing.padding_1)
                        }
